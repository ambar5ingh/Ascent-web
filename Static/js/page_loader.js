(function () {
  ...
  // ── Wait for Plotly to be available (handles slow CDN / network) ──
  function waitForPlotly(timeoutMs = 10000) {
    return new Promise((resolve, reject) => {
      ...
      const t = setInterval(() => {
        if (typeof Plotly !== 'undefined') {
          ...
          clearInterval(t);
          reject(new Error('Plotly failed to load from CDN (network/CSP/firewall issue). Reload the page or check your connection.'));
        }
      }, 50);
    });
  }
  ...
  async function loadPage() {
    try {
      ...
      // Wait for Plotly to be available before doing chart rendering
      await waitForPlotly();
      document.getElementById('loadingOverlay').style.display = 'none';
  ...
  function drawChart(elId, chartJson, extraLayout = {}) {
    ...
    if (typeof Plotly === 'undefined') { console.warn('[ASCENT] Plotly not loaded; skipping chart', elId); return; }
